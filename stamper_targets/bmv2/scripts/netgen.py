...
class MyTopo(Topo):
    def __init__(self, sw_path, json_path, **opts):
        ...
        if use_port_counter > 1 and cfg["dut_ports"][0]["use_port"] == "checked":
            ...
            for dut in cfg["dut_ports"]:
                if dut["use_port"] == "checked":
                    self.addLink("f1", "s1", p4_dev_port, int(re.findall(r'\d+', dut["p4_port"])[0]))
                    p4_dev_port = p4_dev_port + 1
        else:
            print("Only one Dut port used ... Starting 'ReturnIngress' virtual DUT")
            # one port DUT which returns the ingress packets directly without change
            for dut in cfg["dut_ports"]:
                if dut["use_port"] == "checked":
                    self.addNode("f1", cls=ReturnIngress, ip="10.0." + str(dut["id"]) + ".1/24", mac="aa:aa:aa:aa:ff:01")
                    self.addLink("f1", "s1", 1, int(re.findall(r'\d+', dut["p4_port"])[0]))
        ...
        for loadgen_grp in cfg["loadgen_groups"]:
            if loadgen_grp["use_group"] == "checked":
                counter = int(math.ceil(float(counter)/10)*10)  # rounds up to next 10 step
                # if all loadgen groups are less than 10 hosts group 1 gets h10, h11 ... group 2 gets h20, h21..
                for host in loadgen_grp["loadgens"]:
                    ...
                    host["loadgen_ip"] = "10.0." + str(loadgen_grp["group"]) + "." + str(host["id"] + 1)  # ip 1 is saved for DUT(router)
                    print("Setting Group " + str(loadgen_grp["group"]) + " | Host " + str(host["id"]) + " to IP " + host["loadgen_ip"])
                    self.addLink("h" + str(counter), "s1", 1, int(re.findall(r'\d+', host["p4_port"])[0]),
                                 params1={"ip": host["loadgen_ip"] + "/24"})  # load
...
def main():
    ...
    ssh_ips = []
    route_to = ".".join(cfg["p4_dev_ssh"].split(".")[:3]) + ".0/24"
    ...
    for loadgen_grp in cfg["loadgen_groups"]:
        if loadgen_grp["use_group"] == "checked":
            for host in loadgen_grp["loadgens"]:
                ...
                # always autogen MAC
                host["loadgen_mac"] = "22:22:22:22:" + format(loadgen_grp["group"], "02d") + ":" + format(host["id"], "02d")
                host["mn_host_object"].intf(host["mn_name"] + "-eth1").setMAC(host["loadgen_mac"])
                host["mn_host_object"].intf(host["mn_name"] + "-eth1").setIP(host["loadgen_ip"] + "/24")  # if bmv2 docker is used, IP must be set here
                for loadgen_grp2 in cfg["loadgen_groups"]:
                    if loadgen_grp["group"] != loadgen_grp2["group"] and loadgen_grp2["use_group"] == "checked":
                        host["mn_host_object"].cmd("ip route add 10.0." + str(loadgen_grp2["group"]) + ".0/24 via 10.0." + str(loadgen_grp["group"]) + ".1 dev " + host["mn_name"] + "-eth1")
                        print("ip route add 10.0." + str(loadgen_grp2["group"]) + ".0/24 via 10.0." + str(loadgen_grp["group"]) + ".1 dev " + host["mn_name"] + "-eth1")
                host["mn_host_object"].cmd("ip route add " + route_to + " via " + route_via + " dev eth0")
                print("ip route add " + route_to + " via " + route_via + " dev eth0")
    ...
    external_host.cmd("ip route add " + route_to + " via " + route_via + " dev eth0")
    ...
    for dut in cfg["dut_ports"]:
        if dut["use_port"] == "checked" and int(dut["id"]) != 1:
            f1.intf("f1-eth" + str(dut["id"])).setMAC("aa:aa:aa:aa:ff:" + format(dut["id"], "02d"))
            f1.intf("f1-eth" + str(dut["id"])).setIP("10.0." + str(dut["id"]) + ".1/24")
    ...
    for loadgen_grp in cfg["loadgen_groups"]:
        if loadgen_grp["use_group"] == "checked":
            ...
            for host in loadgen_grp["loadgens"]:
                log.append("Host " + str(host["id"]) + ": " + host["mn_name"] + " | " + host["loadgen_iface"] + " " + host["loadgen_ip"] + " " + host["loadgen_mac"] + "\n")
