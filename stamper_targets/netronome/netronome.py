...
class TargetImpl(AbstractTarget):
    ...
    def get_server_install_script(self, user_name, ip, target_specific_dict={}):
        lst = []
        lst.append('echo "====================================="')
        lst.append('echo "Installing Netronome SmartNIC stamper target on '+ip+'"')
        lst.append('echo "====================================="')

        lst.append('if ssh -o ConnectTimeout=2 -o StrictHostKeyChecking=no '+user_name+'@'+ip+' "echo \'ssh to '+ip+' ***worked***\';"; [ $? -eq 255 ]; then')
            
        lst.append('  echo "====================================="')
        lst.append('  echo "\033[0;31m ERROR: Failed to connect to Stamper server '+ip+' \033[0m"')
        lst.append('  echo "====================================="')

        lst.append('else')

        lst.append('  ssh -o ConnectTimeout=2 -o StrictHostKeyChecking=no '+user_name+'@'+ip+' "mkdir -p p4sta/stamper/;"')
        lst.append('  cd '+ self.realPath+'/scripts')
        lst.append('  scp install_nfp.sh ' + user_name + '@' + ip + ':/home/'+user_name+'/p4sta/stamper/' )
        lst.append('  ssh  -o ConnectTimeout=2 -o StrictHostKeyChecking=no '+user_name+'@'+ip+' "cd p4sta/stamper/; chmod +x install_nfp.sh;"')
        lst.append('  ssh  -t -o ConnectTimeout=2 -o StrictHostKeyChecking=no '+user_name+'@'+ip+' "cd p4sta/stamper/; ./install_nfp.sh;"')
        lst.append('  echo "FINISHED setting up Netronome smartNIC stamper target"')
        lst.append('  echo "====================================="')
        lst.append('  echo "\033[1;33m WARNING: Netronome SDE 6.1.0 must be installed manually \033[0m"')
        lst.append('  echo "====================================="')
