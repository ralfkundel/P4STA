...
#include "header_tofino_stamper_v1_2_1.p4"
...
control SwitchEgress(
	...
	action prepare_exthost_packet_udp(bit<48> dst, bit<32> dstip) {
		...
		// TODO: maybe use truncate(new_size) to remove payload ?

	}
	

	action prepare_exthost_packet_tcp(bit<48> dst, bit<32> dstip) {
		// src mac and ip doesnt matter for ext host
		hdr.ethernet.dstAddr = dst;
...
control SwitchEgressDeparser(packet_out packet, inout headers_t hdr, in my_metadata_t meta, in egress_intrinsic_metadata_for_deparser_t eg_dprsr_md) {
	...
	apply {
		...
		#else
			//packet.emit(hdr);
			// TODO: with exthost_udp and NO mac stamping maybe explicit ordering required as it is not parsed (only added for packets egressing to ext host)
