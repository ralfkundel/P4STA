...
control SwitchEgress(
	...
	action add_timestamp1_tcp() {
		...
		hdr.ptp.setValid();
		hdr.ptp.udp_cksum_byte_offset = 8w50 + meta.header_offset; //This is dangerous for additinal headers ...
		hdr.ptp.cf_byte_offset = 8w56 + meta.header_offset; //it's okay to set fixed offset because timestamps are always first tcp option and IPv6 is not supported currently
	...
	action add_timestamp1_udp() {
		...
		hdr.tcp_options_128bit_custom_without.timestamp2 = 48w0x0;
		hdr.ptp.setValid();
		hdr.ptp.udp_cksum_byte_offset = 8w40 + meta.header_offset;
		hdr.ptp.cf_byte_offset = 8w44 + meta.header_offset; //it's okay to set fixed offset because timestamps are always first bytes in udp payload and IPv6 is not supported currently
