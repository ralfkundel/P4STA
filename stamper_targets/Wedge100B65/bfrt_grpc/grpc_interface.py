...
def print_error(error, yellow=False):
    if yellow:
        print_msg = "\033[1;33m"
        print_msg += "-------------------- GRPC Tofino WARNING -------------- \n"
    else:
        print_msg = "\033[1;31m"
        print_msg += "-------------------- GRPC Tofino ERROR ---------------- \n"
...
class TofinoInterface:
    ...
    def __init__(self, grpc_addr, device_id, client_id=randint(1,100), is_master=False):
        ...
        self.grpc_channel = grpc.insecure_channel(grpc_addr, options=[('grpc.max_send_message_length', opt_size), ('grpc.max_receive_message_length', opt_size), ('grpc.max_metadata_size', opt_size)])
        ...
        self.stream_receive_thr = threading.Thread(target=f_stream_receive_thr, args=(self.stream,))
        ...
        stream_request.subscribe.notifications.enable_learn_notifications = True
        stream_request.subscribe.notifications.enable_idletimeout_notifications = True
        stream_request.subscribe.notifications.enable_port_status_change_notifications = True
    ...
    def bind_p4_name(self, p4_program):
        ...
        try:
            ...
            bfrt_info = self.grpc_stub.GetForwardingPipelineConfig(send_request)
    ...
    def _get_full_name(self, name):
        """ Searches in json for given name and creates full name, e.g. "table1" => "pipe.SwitchIngress.table1" """
            ...
            if "key" in table:
                for key in table["key"]:
                    if key["name"].split(".")[-1] == name:
                        print("Replaced key name " + name + " with " + key["name"])
                        return key["name"]
                    ...
                    if "data" in action:
                        for data in action["data"]:
                            if data["name"].split(".")[-1] == name:
                                print("Replaced data name " + name + " with " + data["name"])
                                return data["name"]
        ...
        for i in range(2):
            ...
            for table in self.non_p4_config["tables"]:
                if table["name"] == table_name:
                    return table["id"]
            if i == 0:
                print_error("Table " + table_name + " not found for given p4 program: " + self.p4_program + " try to substitute name with suitable from json.", yellow=True)
                table_name = self._get_full_name(table_name)
            else:
                print_error("Table " + table_name + " not found for given p4 program: " + self.p4_program)
        return None
                ...
                if table["name"] == table_name:
                    for key in table["key"]:
                        ...
                        if key["name"] == key_name:
                            return key["id"], bit_width  # 32 bit fixed size for port stuff?
        else:
            for i in range(2):
                for table in self.bfruntime_info["tables"]:
                    if table["name"] == table_name:
                        for key in table["key"]:
                            if key["name"] == key_name:
                                if table["table_type"] == "Counter" or table["table_type"] == "Register":
                                    if key["type"]["type"] == "uint32":
                                        ...
                                        bit_width = 32
                                        print_error("Bit width for key " + key["name"] + " from table " + table_name + " not determinable - using 32 bit")
                                else:
                    ...
                    for data in table["data"]:
                        if data["singleton"]["name"] == data_name:
                            if data_name == "$MULTICAST_RID":
                                ...
                                bit_width = 32
                            return data["singleton"]["id"], bit_width  # 32 bit fixed size for port stuff?
        else:
            for i in range(2):
                for table in self.bfruntime_info["tables"]:
                    if table["name"] == table_name:
                        if table["table_type"] == "Counter":
                            for data in table["data"]:
                                if data["singleton"]["name"] == data_name:
                                    if data["singleton"]["type"]["type"] == "uint32":
                                        bit_width = 32
                                    elif data["singleton"]["type"]["type"] == "uint64":
                                        bit_width = 64
                                    else:
                                        bit_width = 32
                                        print_error("Bit width for data " + data["name"] + " from table " + table_name + " not determinable - using 32 bit")
                                    return data["singleton"]["id"], bit_width
                        elif table["table_type"] == "Register":
                            for data in table["data"]:
                                if data["singleton"]["name"] == data_name:
                                    return data["singleton"]["id"], data["singleton"]["type"]["width"]
                        else:
                            for action in table["action_specs"]:
                                if action["name"] == action_name:
                                    for data in action["data"]:
                                        if data["name"] == data_name:
                                            return data["id"], data["type"]["width"]
                if i == 0:
                    table_name = self._get_full_name(table_name)
                    action_name = self._get_full_name(action_name)
        return None
    ...
    # add_to_table("t_l1_forwarding", [["ig_intr_md.ingress_port", int(dut["p4_port"])]], [["egress_port", int(loadgen_grp["loadgens"][0]["p4_port"])]], "SwitchIngress.send")
    def add_to_table(self, table_name, keys=[], datas=[], action="", mod_inc=False, silent=False):
        ...
        for key_pair in keys:
            ...
            if type(key_pair[1]) == str:
                ...
                key_field.exact.value = key_pair[1].to_bytes(math.ceil(key_bit_width/8), "big")
            else:
                print_error("Key value must be String or Int! Table: " + table_name + " Key: " + str(key_pair[0]))
        ...
        if action_id is not None:
            ...
            for data_pair in datas:
                ...
                data_field.field_id, data_bit_width = self.get_data_id(data_pair[0], action, table_name)
                if type(data_pair[1]) == int:
                    data_field.stream = data_pair[1].to_bytes(math.ceil(data_bit_width/8), "big")
                    ...
                    if (len(data_pair[1])) > 0 and type(data_pair[1][0]) == int:
                        data_field.int_arr_val.val.extend(data_pair[1])
                    elif (len(data_pair[1])) > 0 and type(data_pair[1][0]) == bool:
                        data_field.bool_arr_val.val.extend(data_pair[1])
                else:
                    print_error("Data value must be Int, Float, String, Bool! Table: " + table_name + " Action: " + action + " Data: " + str(data_pair[0]))

        elif table_name.find("$") == 0 or get_table_type(table_name) == "Counter" or get_table_type(table_name) == "Register":
            for data_pair in datas:
                data_field = tbl_entry.data.fields.add()
                data_field.field_id, data_bit_width = self.get_data_id(data_pair[0], action, table_name)
                if type(data_pair[1]) == int:
                    ...
                    print_error("Data value must be Int, Float, String, Bool, List! Table: " + table_name + " Data: " + str(data_pair[0]))
    ...
    def delete_table(self, table_name):
        try:
            ...
            tbl_entry.table_id = table_id
            tbl_entry.is_default_entry = False          # we don't need to delete default entries because they are default ...
    ...
    def clear_all_tables(self, ignore_tables_list=[]):
        ...
        for table in self.bfruntime_info["tables"]:
            ...
            for elem in ignore_tables_list:
                total_found_indexes = total_found_indexes + table["name"].find(elem)
    ...
    # hosts = [{"p4_port": xxx, "speed": 10G, "fec": "NONE", "an": "default"}, {..}, ..]
    def set_ports(self, hosts):
        def get_an(host):
            try:
                if "an" in host:
                    if host["an"] == "default": return "PM_AN_DEFAULT"
                    elif host["an"] == "on":  return "PM_AN_FORCE_ENABLE"
                    elif host["an"] == "off": return "PM_AN_FORCE_DISABLE"
                    else: raise Exception
                else:
                    raise Exception
            except Exception as e:
                print_error("Error at parsing an-set status for port " + host[
                    "p4_port"] + " setting to 0 (default). This message can occure several times during the port activation process and can be ignored if no AN setting is set. Msg: " + str(
                    e), yellow=True)
        ...
        for host in hosts:
            ...
            try:
                if host["speed"] == "10G":
                    # get "1" from 2/1 hw port because ALL 4 ports must be enabled if breakout cable is used (instead 2/1 all ports (2/0, 2/1, 2/2, 2/3) activated)
                    port_ind = int(host["real_port"].split("/")[-1])
                    if port_ind == 0:
                        p4_ports = [int(host["p4_port"]), int(host["p4_port"]) + 1, int(host["p4_port"]) + 2, int(host["p4_port"]) + 3]
                    elif port_ind == 1:
                        p4_ports = [int(host["p4_port"]) - 1, int(host["p4_port"]), int(host["p4_port"]) + 1, int(host["p4_port"]) + 2]
                    elif port_ind == 2:
                        p4_ports = [int(host["p4_port"]) - 2, int(host["p4_port"]) - 1, int(host["p4_port"]), int(host["p4_port"]) + 1]
                    else:
                        p4_ports = [int(host["p4_port"]) - 3, int(host["p4_port"]) - 2, int(host["p4_port"]) - 1, int(host["p4_port"])]
                    if p4_ports not in already_added_p4_ports:
                        for p4_port in p4_ports:
                            self.add_to_table("$PORT",
                                              keys=[["$DEV_PORT", p4_port]],
                                              datas=[["$SPEED", "BF_SPEED_" + host["speed"]], ["$AUTO_NEGOTIATION", get_an(host)], ["$FEC", "BF_FEC_TYP_" + get_fec(host["fec"], host["speed"])], ["$PORT_ENABLE", True]],
                                              action="")
                        # by adding p4_ports list to already_added.. a second entry_add if host1 has 2/1 and host2 has 2/2 is prevented
                        already_added_p4_ports.append(p4_ports)
                else:
                    self.add_to_table("$PORT", #TODO $DEV_PORT
                                      keys=[["$DEV_PORT", int(host["p4_port"])]],
                                      datas=[["$SPEED", "BF_SPEED_" + host["speed"]],
                                             ["$AUTO_NEGOTIATION", get_an(host)],
                                             ["$FEC", "BF_FEC_TYP_" + get_fec(host["fec"], host["speed"])],
                                             ["$PORT_ENABLE", True]],
                                      action="")
            except:
                print_error(traceback.format_exc())
    ...
    def set_multicast_groups(self, mcast_inp):
        ...
        for item in mcast_inp:
            try:
                mbr_lags = [0]               # non_p4 json repeated = True, list is important!
                mbr_ports = [item["port"]]   # non_p4 json repeated = True, list is important!
                # first create node for port
                self.add_to_table("$pre.node",
                                  keys=[["$MULTICAST_NODE_ID", item["node_id"]]],
                                  datas=[["$MULTICAST_RID", 1], ["$MULTICAST_LAG_ID", mbr_lags], ["$DEV_PORT", mbr_ports]],
                                  action="")
                # now associate node with group (this creates mcast group too)
                node_ids = [item["node_id"]]  # non_p4 json repeated = True, list is important!
                l1_xid_valids = [False]       # non_p4 json repeated = True, list is important!
                l1_xids = [0]                 # non_p4 json repeated = True, list is important!
                self.add_to_table("$pre.mgid",
                                  keys=[["$MGID", item["group_id"]]],
                                  datas=[["$MULTICAST_NODE_ID", node_ids], ["$MULTICAST_NODE_L1_XID_VALID", l1_xid_valids], ["$MULTICAST_NODE_L1_XID", l1_xids]],
                                  action="",
                                  mod_inc=item["group_id"] in created_mcast_grps)
                ...
                print("Added port " + str(item["port"]) + " to node " + str(
                    item["node_id"]) + " and to mcast group " + str(item["group_id"]))
            except:
                print_error("Exception at multicast creating for item: " + str(item))
                print_error(traceback.format_exc())
    ...
    def read_register(self, table_name, register_index=0):
        ...
        key_field.exact.value = register_index.to_bytes(math.ceil(key_bit_width / 8), "big")
        answers = self.grpc_stub.Read(read_request)
    ...
    # reads counter and returns list of tuple(packets, bytes) where index 0 = port 0
    def read_counter(self, table_name, port_list=range(512)):
        ...
        for answer in answers:
            for e in answer.entities:
                as_dict = {}
                for data_field in e.table_entry.data.fields:
                    as_dict[get_name_by_id(data_field.field_id)] = int(data_field.stream.hex(), 16)
        ...
        for elem in counter_read_datas:
            index = port_list[i]
            i = i + 1
            all[index] = (elem["$COUNTER_SPEC_PKTS"], elem["$COUNTER_SPEC_BYTES"])
    ...
    def clear_indirect_counter(self, counter_name, id_list=None):
        def get_counter_size(_counter_name):
            for table in self.bfruntime_info["tables"]:
                if table["table_type"] == "Counter" and table["name"] == _counter_name:
                    return table["size"]
        ...
        for i in id_list:
            try:
                self.add_to_table(counter_name,
                                  ...
                                  datas=[["$COUNTER_SPEC_BYTES", 0], ["$COUNTER_SPEC_PKTS", 0]],
                                  action="",
                                  mod_inc=False,
                                  silent=True)
            except Exception:
