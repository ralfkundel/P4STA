...
parser parserImpl(packet_in packet, out headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
    ...
    state parse_ipv4 {
        ...
        meta.tcp_metadata.tcpLength = hdr.ipv4.paketlen - 16w20;
...
control ingress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
	...
	action add_timestamp2(bit<16> threshold, bit<1> direction) {
		meta.tcp_metadata.threshold_multicast = (bit<16>)threshold; // threshold for multicast how many packets get replicated
		hdr.tcp_options_128bit_custom.timestamp2 = (bit<48>)standard_metadata.ingress_global_timestamp;
		meta.tcp_metadata.added_timestamp2 = 1;
		...
		meta.tcp_metadata.flow_direction = direction;
	...
	action add_timestamp_header(bit<4> new_offset, bit<1> direction) {
		...
		meta.tcp_metadata.tcpLen = meta.tcp_metadata.tcpLen + 16w0x10;
		meta.tcp_metadata.flow_direction = direction;
		meta.tcp_metadata.added_empty_header = 1;
	}
	...
	apply {
		...
		if(hdr.ipv4.isValid()) {
			...
			if(hdr.tcp.isValid()) {
				...
				if(meta.tcp_metadata.added_timestamp2 == 1){
					multi_counter_r.read(meta.tcp_metadata.multi_counter, (bit<32>)meta.tcp_metadata.flow_direction);
					if(meta.tcp_metadata.multi_counter >= meta.tcp_metadata.threshold_multicast){
						multicast.apply();
						meta.tcp_metadata.multi_counter = 0;
						multi_counter_r.write( (bit<32>) meta.tcp_metadata.flow_direction, meta.tcp_metadata.multi_counter);
					} else {
						meta.tcp_metadata.multi_counter = meta.tcp_metadata.multi_counter + 1;
						multi_counter_r.write( (bit<32>) meta.tcp_metadata.flow_direction, meta.tcp_metadata.multi_counter);
					}
				}
...
control egress(inout headers_t hdr, inout metadata_t meta, inout standard_metadata_t standard_metadata) {
	...
	action add_timestamp1() {
		...
		c_add_first_timestamp.count((bit<32>)meta.tcp_metadata.flow_direction);
	}
    ...
    apply {
		broadcast_mac.apply();
		if(hdr.ipv4.isValid()) {
			if(meta.tcp_metadata.added_empty_header == 1) {
				add_timestamp1();
			}
		}
		count_all_egress();
