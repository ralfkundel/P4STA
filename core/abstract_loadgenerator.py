...
class AbstractLoadgenerator:
    ...
    def run_loadgens(self, file_id, duration, l4_selected, packt_size_mtu, results_path, loadgen_rate_limit, loadgen_flows, loadgen_server_groups):
    ...
    def empty_plot(self):
        ...
        to_plot["packetloss"] = {"value_list_input": [0, 0, 0], "index_list": [0, 1, 2],
                                 ...
                                 "filename": "loadgen_3", "adjust_unit": False, "adjust_y_ax": False}
        return to_plot
    ...
    def loadgen_status_overview(self, host, results, index):
        ...
        def check_namespaces(user, ip):
            ...
            for ns in namespaces:
                if ns != "":
                    ns_name = ns.split(" ")[0]
                    answer.append([str(ns)] + P4STA_utils.execute_ssh(user, ip, "sudo ip netns exec " + str(ns_name) + " ifconfig"))
            return answer

        res = {}
        res["ssh_ping"] = (os.system("timeout 1 ping " + host["ssh_ip"] + " -c 1") == 0)
        if res["ssh_ping"]:
            if "namespace_id" in host:
                res["fetched_ipv4"], res["fetched_mac"], res["fetched_prefix"], res["up_state"] = check_iface(host['ssh_user'], host['ssh_ip'], host['loadgen_iface'], host["namespace_id"])
            else:
                res["fetched_ipv4"], res["fetched_mac"], res["fetched_prefix"], res["up_state"] = check_iface(host['ssh_user'], host['ssh_ip'], host['loadgen_iface'], "")
