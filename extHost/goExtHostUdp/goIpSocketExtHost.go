...
func main() {
	...
	var ip_port = flag.String("ip_port", "0.0.0.0:41111", "IP the UDP socket binds to.")
	...
	netaddr, _ := net.ResolveIPAddr("ip4", *ip)
	conn, err := net.ListenIP("ip4:udp", netaddr)
	...
	for {
		...
		bufbyte := make([]byte, 120)
		numRead, _, _ := conn.ReadFrom(bufbyte)
		if err != nil {
			fmt.Println(err)
		...
		buf := bufbyte[:numRead]
		offset := 8 // 8 byte UDP header, if IP socket is used
		...
		if bytes.Equal([]byte{0x0f, 0x10}, buf[2+offset:4+offset]) {
			paket_len_original = binary.BigEndian.Uint16(buf[0+offset : 2+offset])
			timestamp1 = binary.BigEndian.Uint64(append(padding, buf[4+offset:10+offset]...))
			// empty .. buf[10:12]
			timestamp2 = binary.BigEndian.Uint64(append(padding, buf[12+offset:18+offset]...))
