...
class LoadGeneratorImpl(AbstractLoadgenerator):
    ...
    def run_loadgens(self, file_id, duration, l4_selected, packet_size_mtu, results_path, loadgen_rate_limit, loadgen_flows, loadgen_server_groups):
        ...
        def check_iperf_server(server_ssh_user, server_ip, start_port, ns_option):
            # ssh into server and check locally if port is open
            def check_port_open(server_ssh_user, server_ip, port, ns_option):
                answer_list = P4STA_utils.execute_ssh(server_ssh_user, server_ip, str(ns_option) + " if lsof -Pi :" + str(port) + " -sTCP:LISTEN -t >/dev/null; then echo running; fi " + ns_option_end)
            ...
            for add in range(loadgen_flows):
                sys.stdout.write("check iPerf3 Server Port " + str(start_port + add) + " at " + server["ssh_ip"] + " with Namespace " + ns_option + " => ")
                time.sleep(0.5)
                if not check_port_open(server_ssh_user, server_ip, start_port + add, ns_option):
                    time.sleep(1.5)
                    if not check_port_open(server_ssh_user, server_ip, start_port + add, ns_option):
                        sys.stdout.write("[fail]\n")
                        raise Exception("iPerf3 Server Port " + str(start_port + add) + " at " + server["ssh_ip"] + " not open.")
                    else:
                        sys.stdout.write("[ok]\n")
                else:
                    sys.stdout.write("[ok]\n")
        ...
        def start_servers(ssh_user, ssh_ip, start_port, flows, ns_option, server_dict):
            def start_server(ssh_user, ssh_ip, start_port, ns_option):
                P4STA_utils.execute_ssh(ssh_user, ssh_ip, str(ns_option) + " iperf3 -s -p " + str(start_port))
            port = int(start_port)
            for fl in range(flows):
                print("iperf3 server flow " + str(fl) + " start at " + str(ssh_ip) + " port " + str(port) + " with NS option " + str(ns_option))
                server_dict["open_iperf_ports"].append(port)
                thread = threading.Thread(target=start_server, args=(ssh_user, ssh_ip, port, ns_option))
                thread.start()
                port = port + 1
        ...
        def start_clients(ssh_user, ssh_ip, dst_ip, start_port, flows, ns_option, duration, flag, start_json_id):
            def start_client(ssh_user, ssh_ip, dst_ip, port, ns_option, duration, json_id, flag):
                exec = str(ns_option) + " iperf3 -c " + str(dst_ip) + " -T s" + str(json_id) + " -p " + str(port) + " " + flag + " -J --logfile s" + str(json_id) + ".json -t " + str(duration)
                print(exec)
            ...
            for fl in range(flows):
                print("iperf3 client " + str(fl) + " start at " + str(ssh_ip) + " connect to " + str(dst_ip) + " port " + str(port) + " with NS option " + str(ns_option))
                thread = threading.Thread(target=start_client, args=(ssh_user, ssh_ip, dst_ip, port, ns_option, duration, json_id, flag))
        ...
        if loadgen_rate_limit > 0 and num_clients > 0:
            limit_per_host_in_bit_s = int( (loadgen_rate_limit * 1000000)/(loadgen_flows * num_clients) )
            limit_str = "-b " + str(limit_per_host_in_bit_s)
        elif loadgen_rate_limit > 0 and num_clients == 0:
            limit_str = "-b " + str( int( (loadgen_rate_limit*1000000)/loadgen_flows ) )
        else:
            limit_str = ""
        ...
        elif len(iperf_client_groups) == 0 and len(iperf_server_groups) == len(self.cfg["loadgen_groups"]):
            iperf_client_groups = copy.deepcopy(iperf_server_groups)
        ...
        for server_group in iperf_server_groups:
            for server in server_group["loadgens"]:
                for i in range(num_clients):
                    start_servers(server["ssh_user"], server["ssh_ip"], str(start_port), loadgen_flows, check_ns(server), server_dict=server)
        ...
        for server_group in iperf_server_groups:
            for server in server_group["loadgens"]:
                for i in range(num_clients):
                    check_iperf_server(server["ssh_user"], server["ssh_ip"], start_port, check_ns(server))
                    start_port = start_port + loadgen_flows
            ...
            for client in client_group["loadgens"]:
                ...
                for server_group in iperf_server_groups:
                    for server in server_group["loadgens"]:
                        start_client = False
                        do_break = False
                        # if more clients in client grp than server grp connect remaining clients to first servers again
                        if len(client_group["loadgens"]) >= len(server_group["loadgens"]):
                            if server["id"] == custom_modulo(client["id"], len(server_group["loadgens"])) and client["loadgen_ip"] != server["loadgen_ip"]:
                                start_client = True
                                do_break = True
                        else:
                            if client["id"] == custom_modulo(server["id"], len(client_group["loadgens"])) and client["loadgen_ip"] != server["loadgen_ip"]:
                                start_client = True
                                do_break = False
                        ...
                        if start_client:
                            if len(server["open_iperf_ports"]) > 0:
                                ...
                                for i in range(loadgen_flows):
                                    if start_port+i in server["open_iperf_ports"]:
                                        server["open_iperf_ports"].remove(start_port+i)
                                print("select port range starting at " + str(start_port) + " to connect from " + client["loadgen_ip"] + " to " + server["loadgen_ip"])
                                x = threading.Thread(target=start_clients, args=(client["ssh_user"], client["ssh_ip"], server["loadgen_ip"], start_port, loadgen_flows, check_ns(client), duration, flag, json_id))
                                threads.append(x)
                                json_id = json_id + loadgen_flows
                                client["num_started_flows"] = client["num_started_flows"] + loadgen_flows
                                if do_break:
                                    break
                            else:
                                print("No available port found in server dict: " + str(server) + " from server group id " + str(server_group["group"]))
        ...
        for client_grp in iperf_client_groups:
            for client in client_grp["loadgens"]:
                for f in range(client["num_started_flows"]):
                    exc = "scp " + client["ssh_user"] + "@" + client["ssh_ip"] + ":s" + str(
                        json_id + f) + ".json " + results_path + "/iperf3_s" + str(json_id + f) + "_" + str(
                        file_id) + ".json"
                    print(exc)
                    subprocess.run(exc, shell=True)
                json_id = json_id + client["num_started_flows"]

        # delete in second loop because e.g. in case of docker one rm -f deletes all jsons for all clients
        for client_grp in iperf_client_groups:
            for client in client_grp["loadgens"]:
                P4STA_utils.execute_ssh(client["ssh_user"], client["ssh_ip"], "rm -f s*.json")
    ...
    def process_loadgen_data(self, file_id, results_path):
        ...
        for json_dict in jsons_dicts:
            if len(json_dict["interval_mbits"]) < lowest_mbit_len or lowest_mbit_len == -1:
                lowest_mbit_len = len(json_dict["interval_mbits"])
        for json_dict in jsons_dicts:
            json_dict["interval_mbits"] = json_dict["interval_mbits"][0:lowest_mbit_len]
        for json_dict in jsons_dicts:
            if l4_type == "tcp":
        ...
        if l4_type == "tcp":
            ...
            try:
                ...
                for i in list(s["intervals"]):
                    ...
                    # udp does not support intervall packet loss -> even distribution
    ...
    def get_server_install_script(self, list_of_server):
        ...
        for server in list_of_server:
            ...
            lst.append('  echo "====================================="')
            lst.append('  echo "\033[0;31m ERROR: Failed to connect to loadgen server '+ip+' \033[0m"')
            lst.append('  echo "====================================="')
            ...
            lst.append('  ssh  -o ConnectTimeout=2 -o StrictHostKeyChecking=no ' + user_name + '@' + ip + ' "mkdir -p /home/' + user_name + '/p4sta/loadGenerator/iperf3;"')
            lst.append('  scp install_iperf3_server.sh ' + user_name + '@' + ip + ':/home/' + user_name + '/p4sta/loadGenerator/iperf3')
            lst.append('  ssh  -o ConnectTimeout=2 -o StrictHostKeyChecking=no ' + user_name + '@' + ip + ' "cd /home/' + user_name + '/p4sta/loadGenerator/iperf3; chmod +x install_iperf3_server.sh;"')
            lst.append('  ssh  -t -o ConnectTimeout=2 -o StrictHostKeyChecking=no ' + user_name + '@' + ip + ' "cd /home/' + user_name + '/p4sta/loadGenerator/iperf3; ./install_iperf3_server.sh;"')
